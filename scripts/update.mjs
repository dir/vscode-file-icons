...
const isOwn = Object.prototype.hasOwnProperty.call.bind(
...
const missing = resolve(
	process.argv[4] || join(root, "scripts", "missing-filenames.txt")
);
...
export default Promise.all([
	...
	.then(async ([iconDB, icons, fonts, colours]) => {
		...
		console.info(
			count ? `${count} font(s) updated` : "Fonts already up-to-date"
		);
		...
		for (const font of fonts) font.id = icons.fonts[font.id].id || font.id;
		...
		saveJSON(
			uncolouredTheme,
			join(output, "file-icons-colourless-icon-theme.json")
		);
	})
	.catch((error) => {
		console.error(error);
...
function buildTheme({ iconDB, icons, fonts, colours, prefix = "_" } = {}) {
	...
	const getColourValue = (colour) => {
		...
		if (null == value) throw new ReferenceError(`No such colour ${colour}`);
		return value;
	};
		...
		for (let [
			...
			const add = (listName, key) => {
				...
				for (const colour of colours) {
					...
					if (null == theme.iconDefinitions[uid]) {
						...
						if ("#000000" === def.fontColor) delete def.fontColor;
			...
			try {
				...
				if (scope) {
					...
					const iconID =
						prefix + icon + (colours.length ? "_" + colours[0] : "");
					...
					if (language) {
						language = parseRegExp(
							(match = new RegExp(language.source, language.flags))
						);
						langIDs.push(
	...
	for (const obj of [theme, theme.light].filter(Boolean))
		for (const key of "iconDefinitions fileExtensions fileNames folderNames folderNamesExpanded languageIds".split(
			...
			const value = obj[key];
			if (isOwn(obj, key) && isObj(value)) obj[key] = sortProps(value);
		}
	return theme;
}
...
function desaturate(input, colours) {
	...
	for (const [oldID, props] of Object.entries(oldDefs)) {
		...
		// Sanity check
		if (newID in newDefs) assert.deepStrictEqual(props, newDefs[newID]);
	...
	const cull = (...listNames) => {
		for (const listName of listNames) {
			if (!isObj(result?.light?.[listName]) || !isObj(result[listName]))
				continue;
			...
			if (isEmpty(result.light[listName])) delete result.light[listName];
		}
		if (isEmpty(result.light)) delete result.light;
	};
	...
	if (size) {
		const [prefix, path, dimOn, dimOff] = process.stderr.isTTY
			? [
					...
					`\x1B[4m${filenameList}\x1B[24m`,
					"\x1B[2m",
					"\x1B[22m",
			  ]
			: ["Warning:", filenameList];
		console.warn(
			`${prefix} Unmatched ${1 === size ? "entry" : "entries"} in ${path}:`
		);
...
function parseRegExp(input) {
	...
	if ("genex" !== input?.constructor?.name.toLowerCase()) input = Genex(input);
	...
	const walk = (obj, refs = new WeakSet()) => {
		if ("object" !== typeof obj || null === obj || refs.has(obj)) return;
		refs.add(obj);
		if (Array.isArray(obj)) {
			...
			if (Infinity === obj.max) {
				if (!obj.min) throw new RangeError("Bad range");
				obj.max = ~~obj.min;
			} else
				switch (obj.type) {
					case 2:
	...
	for (const token of killList) {
		for (const list of lists) {
			while (list.includes(token)) list.splice(list.indexOf(token), 1);
		}
	}

	const used = String.fromCodePoint(...chars);
	const unused = Array.from(getUnusedChar(used + "\\[]{}()?+*", 2)).map(
	...
	if (name in icons) {
		...
		if (!fonts.some((font) => fontId === font.id))
			throw new ReferenceError(
				`Icon "${name}" references undefined font "${fontId}"`
			);
...
async function loadIcons(from) {
	...
	for (const selector in rules) {
		...
		if (!font) continue;
		if (
			/^\.((?:(?!-|\d)[-a-z0-9]+|_\d+[-a-z0-9]*)(?<!-))(?:::?before)?$/i.test(
				selector
			)
		) {
			const name = RegExp.$1.replace(/^_|-icon$/gi, "");
			if ("string" === typeof rule.content) icons[name] = rule;
	...
	for (const name in icons) {
		...
		if (!font) throw new TypeError(`No font defined for icon '${name}'`);
		if (!(font in fonts)) throw new TypeError(`Undefined font ${font}`);
		...
		if ("100%" !== size) icons[name].fontSize = size;
	}
	...
	switch (typeof input) {
		...
		case "string":
			...
			switch (unit) {
				...
				case "rem":
					value *= 16;
					break; // Root em (assumed to be 16px)
				case "%":
					return `${Math.round(value)}%`; // Percentage of font-size
				default:
		...
		if (Array.isArray(src)) {
			if (Array.isArray(src[0]) && "local" === src[0][0])
				src =
					src.find(
						(x) =>
							"string" === typeof x || (Array.isArray(x) && "local" !== x[0])
					) ?? src;
			if ("string" === typeof src) path = src;
			else
				for (const item of src) {
					if (null == path && "string" === typeof item) path = item;
...
function updateFonts(fontDefs, targetDir, { force, noLink } = {}) {
	...
	for (const font of fontDefs) {
		...
		if (!exists(dstPath)) {
			const [str, fn] =
				srcStat.dev !== stat(targetDir).dev
...
function assertDir(...paths) {
	for (const path of paths) {
		...
		if (!stats) throw new Error("No such directory: " + path);
...
function parse(node, refs = new WeakSet()) {
	...
	if (!value && (value = rules || args)) {
		...
		if (value.every((item) => isEnt(item))) value = Object.fromEntries(value);
...
function parseRules(ruleset) {
	...
	for (const rule of ruleset.rules) {
		if (!rule || "Comment" === rule.type) continue;
		const selectors = rule.selectors.map((sel) =>
			sel.elements
				.map((el) => el.combinator.value + el.value)
				.join("")
				.trim()
		);
